        ...
        <table className="w-full text-sm">
          <thead>
            <tr>
              ...
              <SortHeader col="pilot_count" label={t("pilots")} align="right" activeSort={sortKey} activeDir={sortDir} onSort={(key) => toggleSort(key as SortKey)} />
              <SortHeader col="xc_potential" label={t("xcPotential")} align="right" activeSort={sortKey} activeDir={sortDir} onSort={(key) => toggleSort(key as SortKey)} />
              ...
              <SortHeader col="record_km" label={t("record")} align="right" activeSort={sortKey} activeDir={sortDir} onSort={(key) => toggleSort(key as SortKey)} />
              <SortHeader col="last_activity" label={t("lastActivity")} activeSort={sortKey} activeDir={sortDir} onSort={(key) => toggleSort(key as SortKey)} />
              <SortHeader col="weekend_pct" label={t("weekendPct")} align="right" activeSort={sortKey} activeDir={sortDir} onSort={(key) => toggleSort(key as SortKey)} />
            </tr>
          </thead>
            ...
            {sorted.slice(0, 100).map((tk) => {
              ...
              return (
                <tr key={tk.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-2 py-2 text-gray-700 text-right">{tk.flight_count}</td>
                  <td className="px-2 py-2 text-gray-700 text-right">{tk.pilot_count}</td>
                  <td className="px-2 py-2 text-gray-700 text-right">{tk.xc_potential ? `${tk.xc_potential} km` : "-"}</td>
                  <td className="px-2 py-2"><MiniSparkline data={tk.monthly_data} /></td>
                  <td className="px-2 py-2 text-gray-700 text-right">{tk.record_km ? `${Number(tk.record_km).toFixed(1)}` : "-"}</td>
                  <td className="px-2 py-2 text-gray-500 text-xs">
                    {tk.last_activity ? formatDate(tk.last_activity, locale) : "-"}
                  </td>
                  <td className="px-2 py-2 text-gray-700 text-right">{tk.weekend_pct != null ? `${tk.weekend_pct}%` : "-"}</td>
                </tr>
              );
            })}
          </tbody>
