...
export default async function WingsPage() {
  ...
  const tableData = (wings as any[]).map((w) => ({
    ...
    total_km: w.total_km ? Number(w.total_km) : 0,
    avg_distance: w.avg_distance ? Number(w.avg_distance) : null,
    max_distance: w.max_distance ? Number(w.max_distance) : null,
    avg_speed: w.avg_speed ? Number(w.avg_speed) : null,
